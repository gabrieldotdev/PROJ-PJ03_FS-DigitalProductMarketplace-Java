<%@ page contentType="text/html;charset=UTF-8" language="java" %>

<aside id="sidebar" class="mt-4 border-r border-slate-900/10 px-8 py-2">
    <!-- Sidebar content here -->
    <div class="h-full">
        <ul>
            <li>
                <a href="/" class="group mb-4 flex items-center font-semibold text-indigo-500 lg:text-sm lg:leading-6">
                    <lord-icon src="https://cdn.lordicon.com/osuxyevn.json" trigger="hover" class="h-6 w-6"> </lord-icon>
                    <span id="sidebar-home" class="ml-4">Trang chủ </span>
                </a>
            </li>
            <li>
                <a href="/" class="group mb-4 flex items-center font-semibold text-slate-800 hover:text-indigo-500 lg:text-sm lg:leading-6">
                    <lord-icon src="https://cdn.lordicon.com/isugonwi.json" trigger="hover" class="h-6 w-6"> </lord-icon>
                    <span id="sidebar-about" class="ml-4">Giới thiệu </span>
                </a>
            </li>
            <li>
                <a href="/" class="group mb-4 flex items-center font-semibold text-slate-800 hover:text-indigo-500 lg:text-sm lg:leading-6">
                    <lord-icon src="https://cdn.lordicon.com/uiakkykh.json" trigger="hover" class="h-6 w-6"> </lord-icon>

                    <span id="sidebar-admin" class="ml-4">Quản lý hệ thống </span>
                </a>
            </li>
        </ul>
    </div>
</aside>